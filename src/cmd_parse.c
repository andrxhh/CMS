...
/*
This file contains parsing helpers for command argument strings.
*/
...
bool parse_args_to_patch(char *args, Student *patch) {
    // Current scanning pointer into args
    char *p = args;
    ...
    while (p && *p) {
        ...
        // Malformed if no '=' or non-space chars between key and '='
        if (!eq || check_ptr != eq) {
        ...
        // value_end is the start of the next key, so the value
        // spans value_start to value_end or until the end of the string.
        const char *next_key = NULL;
        char *value_end = find_next_key(value_start, &next_key);

        // Copy value into a temporary buffer and trim it. Upper bound length enforced.
        char value_buf[256];
        size_t len;
        if (value_end == NULL) {
        ...
        // If value is quoted strip it.
        // If quotes are unmatched, report error and stop parsing.
...
// Strictly only parse single ID argument in the form ID=<number>
bool parse_single_id_command(char *args, const char *cmd_name, int *out_id) {
    ...
    // Move past "ID"
    p += 2;
    while (isspace((unsigned char)*p)) p++; // Skip any whitespace between ID and '='
    ...
    // Move past '='
    p++;
    while (isspace((unsigned char)*p)) p++; // Skip leading whitespace between '=' and value

    // If no value present, error
    if (*p == '\0') {
    ...
    // If any non-space chars remain, report error
    if (*endptr != '\0') {
